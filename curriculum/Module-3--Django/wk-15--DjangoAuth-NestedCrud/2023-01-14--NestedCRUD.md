...
Let's create an nested CRUD application today called `cohorts_and_students`. In this challenge, we will CRUD a `cohort` and its associated `students`. Let's go with Code Platoon's Bravo and delta platoons. Starting off, we'll create an app:
...
delta = Cohort(cohort_name='delta', start_date='5/27/17', end_date='8/10/17')
delta.save()
...
student_5 = Student(cohort=delta, first_name='Darnell', last_name='G')
student_5.save()

student_6 = Student(cohort=delta, first_name='Arthur', last_name='M')
student_6.save()
```
...
Finally, let's get the `templates/cohorts_and_students/` folder going and get some files in there:
...
In `templates/cohorts_and_students/cohort_detail.html`, add this code to the end:
```html
<hr/>
<div>
...
From here, let's create some templates in `templates/students/`:
```html
# student_detail.html
<div>
